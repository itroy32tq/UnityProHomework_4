...
namespace UniRx
{
    ...
    internal sealed class ObserverNode<T> : IObserver<T>, IDisposable
    {
        ...
        public void ChangeList(IObserverLinkedList<T> listToChange)
        {
            list = listToChange;
        }
    ...
    public class ReactiveProperty<T> : IReactiveProperty<T>, IDisposable, IOptimizedObservable<T>, IObserverLinkedList<T>
    {
        ...
        public void DisposeAndTransferSubscriptionsTo(ReactiveProperty<T> target)
        {
            if (root == null)
            {
                return;
            ...
            while (node != null)
            {
                node.ChangeList(target);
                node = node.Next;
            }
            
            target.isDisposed = isDisposed;
            if (target.root == null)
            {
                target.root = root;
                target.last = last;
            ...
            {
                target.last.Next = root;
                root.Previous = target.last;
                target.last = last;
            }
    ...
    {
        public static IDisposable SubscribeTo<T>(this ReactiveProperty<T> from, IReadOnlyReactiveProperty<T> to)
        {
            bool changed = !Equals(from.Value, to.Value);
            IDisposable disposable = to.Subscribe(value => from.Value = value);
            if (changed)
            {
                from.SetValueAndForceNotify(to.Value);
            }
